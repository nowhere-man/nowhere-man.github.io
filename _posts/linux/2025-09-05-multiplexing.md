...
2.同时能够监听的文件描述符数量太少。受限于sizeof(fd_set)的大小，在编译内核时就确定了且无法更改。一般是32位操作系统是1024，64位是2048。
...
1.  **`epoll_ctl()`**：
    ...
    +   当调用 `epoll_ctl()` 并添加一个文件描述符 `fd` 时，内核会将该 `fd` 和你指定的事件（`events`）一起，作为一个节点插入到**红黑树**中。
    +   同时，内核会为这个 `fd` 关联一个**回调函数**。

1.  **`epoll_wait()`**：

    +   当调用 `epoll_wait()` 时，它会检查内核中的**就绪列表**。
    +   如果就绪列表不为空，它会立即返回，将就绪列表中的事件**拷贝**到传入的用户空间数组中。
    +   如果就绪列表为空，`epoll_wait()` 会阻塞进程，直到有事件发生。

1.  **事件发生与唤醒**：

    +   如果一个socket收到了数据。内核检测到这个事件后，它会触发之前注册好的**回调函数**。
    +   回调函数会将这个已就绪的文件描述符从红黑树中找到，并将其添加到**就绪列表**中。
    +   同时，回调函数会唤醒所有在 `epoll_wait()` 上阻塞的进程。
